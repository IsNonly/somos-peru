...
export default function PanelGeneral() {
  ...
  useEffect(() => {
    ...
    if ((esDistrital || esPCV) && miAmbito.distrito) { setFDist(miAmbito.distrito); return }
    if (esProvincial) {
      setFDepto(miAmbito.departamento || AMBITO_DEPARTAMENTO)
      setFProv(miAmbito.provincia || '')
    }
  }, [ambitoListo, esDistrital, esPCV, esProvincial, miAmbito.departamento, miAmbito.provincia, miAmbito.distrito])
  ...
  const bloqueado = (campo: 'depto' | 'prov' | 'dist') => {
    if (esDistrital || esPCV) return true
